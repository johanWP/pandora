        ...
        Excel::create('Movimientos de Artículos por Almacen', function($excel) use ($desde,$hasta,$actividad,$company_id) {
            ...
            $sheet->setColumnFormat(array(
				...
				foreach($movements as $key => $value)
				{
				  ...
				  if($value['status_id']==1) { $movements[$key]['status_id'] = 'Aprobado'; }
				  if($value['status_id']==2) { $movements[$key]['status_id'] = 'Por Aprobar'; }
				  if($value['status_id']==3) { $movements[$key]['status_id'] = 'Eliminado'; }
				  if($value['status_id']==4) { $movements[$key]['status_id'] = 'Rechazado'; }
